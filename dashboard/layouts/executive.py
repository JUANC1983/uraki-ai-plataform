...
def render() -> None:
    ...
    for case in cases:
        ...
        row[0].markdown(f"""<div style="font-size:11px;color:{c['text_muted']};
            opacity:{opacity};padding:0.35rem 0;font-variant-numeric:tabular-nums;">
            {case.get('id','—')}</div>""", unsafe_allow_html=True)

        row[1].markdown(f"""<div style="font-size:12px;color:{c['text_primary']};
            opacity:{opacity};padding:0.35rem 0;font-weight:500;
            white-space:nowrap;overflow:hidden;text-overflow:ellipsis;">
            {case.get('client_name','—')}</div>""", unsafe_allow_html=True)

        days = case.get("overdue_days", 0)
        row[2].markdown(f"""<div style="font-size:12px;
            color:{'#EF4444' if days > 90 else '#F59E0B' if days > 30 else c['text_primary']};
            opacity:{opacity};padding:0.35rem 0;font-weight:600;">{days}d</div>""",
            unsafe_allow_html=True)

        row[3].markdown(f"""<div style="font-size:11px;color:{c['text_secondary']};
            opacity:{opacity};padding:0.35rem 0;font-variant-numeric:tabular-nums;">
            ${case.get('overdue_amount',0)/1_000_000:.1f}M</div>""", unsafe_allow_html=True)
        ...
        <div style="padding:0.35rem 0;opacity:{opacity};">
            <span style="background:rgba({_hex_to_rgb(s_color)},0.10);color:{s_color};
                ...
                {STATUS_LABELS.get(case.get('status',''), case.get('status',''))}</span>
        </div>""", unsafe_allow_html=True)
        ...
        with row[6]:
            if not is_closed:
                ...
                st.markdown(f'<div style="padding:0.35rem 0;font-size:11px;'
                            f'color:{c["text_muted"]};">Cerrado</div>', unsafe_allow_html=True)
